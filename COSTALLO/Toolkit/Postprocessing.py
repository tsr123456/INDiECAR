...
def Run_postprocessing (path, name_run, country):
    ...
    backcasting = 0 # for backcasting NEts allsocation does not work:
    if backcasting:
        average_LCOP_cement = sum(df_grid_filtered_cement["LCOP_total"]*df_grid_filtered_cement["capacity"])/sum(df_grid_filtered_cement["capacity"]) + (opex_nets_total+annualise_CAPEX(capex_nets_total))/sum(df_grid_filtered_cement["capacity"])

    #--------------------------------------------- Plot results as MACCs -------------------------------------------------------------------#
    
    #sort results by costs:
    df_grid_filtered_cement.sort_values("LCOP_total_w_nets", ascending=True, inplace=True)

    #calculate the cummulative emission reduction
    ...
    #reset index to avoid plotting errors: 
    ...
    labels = {
    "CCS_CEMENT":"CCS",
    "BIOMASS_CEMENT":"Biofuel",
    "CCS_BIOMASS_CEMENT":"CCS, biofuel",
    "MINERAL_CEMENT":"$CO_{2}$ mineralisation", 
    "BIOMASS_MINERAL_CEMENT":"$CO_{2}$ mineralisation, biofuel",
    "CCS_BIOMASS_MINERAL_CEMENT":"CCS, $CO_{2}$ mineralisation, biofuel",
    "CCS_MINERAL_CEMENT":"CCS, $CO_{2}$ mineralisation",
    "CALCINED_CLAY_CEMENT":"Calcined clay",
    "CALCINED_CLAY_BIOMASS_CEMENT":"Calcined clay, biofuel",
    "CCS_CALCINED_CLAY_CEMENT":"CCS, calcined clay",
    "CCS_CALCINED_CLAY_BIOMASS_CEMENT":"CCS, calcined clay, biofuel",
    "NETS":"DACCS"}
    ...
    textstr =(f"TCR transport and storage: $€${round(capex_network_total/1000000)} $M$\n"
        ...
        f"TCR direct air capture: $€${round(capex_nets_total/1000000)} $M$\n"
    ...
    # Save to shapefile:
    save_tech_invest =True
    save_net_invest = True
