...
MAX_STRG_TECH_PER_CELL = 100
#MAX_END_USE_TECH_PER_CELL = 100
ANNUALISATION_TIME = 30 #years
...
def eqnR2_CellBalance(m, r, g, t, tm, s):
    ...
    for pt in m.PROCESS_TECH:
        ...
        if (r in m.R_POLLUTANT) & (conv < 0):
            ...
            conv = 0
        # catch all transported feedstocks:    
        if r == name_for_emitted_CO2:
            #for each transportable good, add location specific transport emissions: 
            for r2 in m.TRAN_R:
                trans_emiss = m.TRANSPORT_EMISS[r2,g]
                conv += -m.RESOURCE_CONV_RATE[pt,r2]*m.TRANSPORT_EMISS[r2,g]
    ...
    #for g in m.G:
        #for r in m.R_POLLUTANT:
            #for r1 in m.TRAN_R:
    #            rate = m.transport_rate[r1, g, t, tm, s]
    #            stack += m.TRANSPORT_EMISS[r1,g] * rate #* time
    #print(stack)
    return m.emission_rate[r, g, t, tm, s] == stack


#def eqnR2_add_transport_emissions(m,r,g,t,tm,s):
#    "Add emissions from transport to grid emissions for each grid by updating the grid's emission rate"
#    #change emission rate for emitted CO2 by adding tranpsort emissions:
#    rate = m.transport_rate[r, g, t, tm, s]
#    #print(rate)
#    if (r in m.TRAN_R) & (m.IMPORT_LOCATIONS[g] == 1):
#       #time = m.OPER_TIME_INTERVAL[t]
#        rate = m.transport_rate[r, g, t, tm, s]
#        emission_rate = m.emission_rate[name_for_emitted_CO2,g,t,tm,s]
#        emission_rate += m.TRANSPORT_EMISS[r,g] * rate #* time
#        print(f'in r:{emission_rate}')
#    #if resource is not a transportable good, don't do anything: 
#    else:
#        emission_rate = m.emission_rate[name_for_emitted_CO2,g,t,tm,s]
#    return m.emission_rate[name_for_emitted_CO2 , g, t, tm, s] == emission_rate                     
...
def eqnJ3_process(m, process_tech, g, t, tm, s):
    ...
    #Important for this constraint to work, we here can only allow one process intervention per cell. 
    N = m.num_process[process_tech, g, tm, s]
    max_rate = m.NAME_PLATE_CAP[process_tech]
    max_rate *= m.TECH_AVAILABILITY[process_tech, tm]
    min_rate = m.MIN_CAP[process_tech]
    min_rate *= m.TECH_AVAILABILITY[process_tech, tm]
    return rate <= N * max_rate
...
def eqnJ3_enduse(m, end_use_tech, g, t, tm, s):
    ...
    min_rate = m.MIN_CAP[end_use_tech]
    min_rate *= m.TECH_AVAILABILITY[end_use_tech, tm]
    return rate <= N * max_rate
...
def eqnS4_CO2(m, g, co2_storage_tech, t, tm, s):
    ...
    cap = MEGATON_TO_TON * m.TOT_CO2_STORES[g]/ANNUALISATION_TIME
